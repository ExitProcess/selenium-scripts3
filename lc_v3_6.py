...
con = sqlite3.connect("links_v3.6.db")
...
while count < last_id:
    ...
    # обработчик ошибок HTTP
        ...
        with con:
            cur = con.cursor()
            for new_link in new_links:
                cur.execute("SELECT Id FROM Links WHERE Link=? OR Link=? OR Link=?", (new_link, new_link + "/", new_link[:-1]))
                child_link_id = cur.fetchone()[0]
                parent_child = (first_not_parsed_link_id, child_link_id)
                parent_child_list.append(parent_child)
            cur.executemany("INSERT INTO Parent_child (Parent_link, Child_link) VALUES (?, ?)", parent_child_list)
        time0 = time.time() - time0
        time2 += time0
        ...
        print("первая таблица обработана: %s" % time1)
        print("вторая таблица обработана: %s" % time2)
        time_all_temp = time.time() - time_all_temp
        time_all += time_all_temp
        print("прошло времени: %s" % time_all)
        print("скорость: %s ссылок/сек" % (count / time_all))
...
# v3.5 -- оптимизация. значительно уменьшено количество запросов к БД.
# v3.6 -- оптимизация. with ... as вынесен из тела циклов. добавлены счетчики.
