...
con = sqlite3.connect("links_v3.5.db")
    ...
    # обработчик ошибок HTTP
        ...
        for new_link in new_links:
            with con:
                ...
                child_link_id = list(child_link[0])[0]
...
# v3.5 -- оптимизация. значительно уменьшено количество запросов к БД (cur.executemany вместо cur.execute).
