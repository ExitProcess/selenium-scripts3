# надо выяснить, располагаются ли элементы в списке по возрастанию
# 1 метод -- перебор всех элементов списка в цикле, последний элемент проверяется вне цикла
# 2 метод -- перебор всех элементов списка в цикле через try / except
# 3 метод -- создать копию проверяемого списка, выполнить list.sort() и сравнить полученный список с оригинальным
#            если списки равны -- то элементы в проверяемом списке расположены по возрастанию
#            (если проверяется убывание, то после list.sort() надо сделать list.reverse())
# в этом скрипте проверяется скорость всех методов
