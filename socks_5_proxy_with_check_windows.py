# написано в учебных целях -- получение некоего значения на одном сайте(окне) и передача значения в форму
#   на другом сайте(открытым во второй окне) -- хендлы и т.д.
# в отличие от socks_5_proxy_with_check_handlers.py, в этом скрипте используются окна, а не вкладки
...
# подготовка второго окна
# shift + лкм по любому элементу
# (ctrl + N не работает, проблема описана в test_ctrl_t.py)
temp_elem = driver.find_element_by_link_text("SPYS.ONE © 2008-2018")
ActionChains(driver).key_down(Keys.LEFT_SHIFT).click(temp_elem).key_up(Keys.LEFT_CONTROL).perform()
