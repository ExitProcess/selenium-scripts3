    ...
    try:
        ...
        for link in links_list:
            if "ftp" not in link and "#" not in link and "www.python.org" in link:
                if link not in links_base and (link + '/') not in links_base and link[:-1] not in links_base:
                    # для того, чтобы работал метод make_links_absolute, ссылка должна заканчиваться слешем
                    # "?" not in link - https://www.python.org/jobs/?page=1 -- не добавлять, т.к. запрос
                    # не добавлять слеш к расширениям -- .asc, .txt, .png, .psd, .svg, .pdf
                    if link[-1] != "/" and link[-4] != "." and link[-5] != "." and "?" not in link:
                        link = link + "/"
